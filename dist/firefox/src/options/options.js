...
function setupEventListeners() {
    ...
    $('#remove-global-perm').addEventListener('click', async () => {
        const removed = await permissionsRemove({ origins: ['<all_urls>'] });
...
async function showGrantBanner(origins) {
    const banner = document.getElementById('grant-banner');
    const text = document.getElementById('grant-banner-text');
    const status = document.getElementById('grant-banner-status');
    const approve = document.getElementById('grant-banner-approve');
    const cancel = document.getElementById('grant-banner-cancel');
    if (!banner || !approve || !cancel) return;

    pendingGrant = Array.isArray(origins) ? origins : [];
    if (!pendingGrant.length) return;

    text.textContent = `The extension requests access to: ${pendingGrant.join(', ')}`;
    status.textContent = '';
    ...
    approve.onclick = async () => {
        ...
        try {
            let granted = await permissionsRequest({ origins: pendingGrant });
            if (!granted && pendingGrant.length > 1) {
                for (const o of pendingGrant) {
                    const ok = await permissionsRequest({ origins: [o] });
                    if (ok) granted = true;
                }
            }
            status.textContent = granted ? 'Granted.' : 'Not granted or dismissed.';
            if (granted) {
                await storageSet({ 'cookiecontrol:pending-origins': [] });
...
async function checkPendingGrant() {
    try {
        const res = await storageGet(['cookiecontrol:pending-origins']);
        const arr = res && res['cookiecontrol:pending-origins'];
...
async function init() {
    ...
    setupEventListeners();
    setupThemeSelector();
    // If Options page is already open when popup triggers a grant, react to storage changes
    try {
        chrome.storage.onChanged.addListener((changes, area) => {
            const key = 'cookiecontrol:pending-origins';
            const ch = changes[key];
            if (ch && Array.isArray(ch.newValue) && ch.newValue.length) {
                showGrantBanner(ch.newValue);
            }
        });
    } catch (_) { /* ignore */ }
    await checkPendingGrant();
    loadGrantedOrigins();
    updateGlobalPermissionStatus();
    loadLog();
}
